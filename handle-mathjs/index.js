...
// const a = math.rotate([11, 12], math.pi / 2)
...
function getCssProperty(targetDom, cssPropertyName) {
  return window.getComputedStyle(targetDom)[cssPropertyName];
}

function reflectParameter() {
  const itemDom = document.querySelector('.item');
  itemDom.style.transform = `translateX(${parameter.translateX}px) translateY(${parameter.translateY}px) rotate(${parameter.rotate}deg)`;
  console.log(getCssProperty(itemDom, 'transform'));
}
...
gui.width = 300;
gui.add(controllerInfo, 'Translate X', -500, 500, 1).onChange((event) => {
  detectChangeParameter(event, 'Translate X');
});
gui.add(controllerInfo, 'Translate Y', -500, 500, 1).onChange((event) => {
  detectChangeParameter(event, 'Translate Y');
});
gui.add(controllerInfo, 'Rotate', -360, 360, 1).onChange((event) => {
  detectChangeParameter(event, 'Rotate');
});
